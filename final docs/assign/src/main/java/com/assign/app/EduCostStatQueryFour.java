...
/*db.colleges.aggregate([
  ...
  },
  {
    $group: {
      ...
      latestYear: { $max: "$_id.Year" },
      baseYearTotalExpense: {
        $sum: {
          $cond: {
            if: { $eq: ["$_id.Year", "$Year"] },
            then: "$totalExpense",
    ...
    $addFields: {
      growthRate: {
        $map: {
          input: {
            $range: [
              { $subtract: ["$latestYear", 5] },
              { $subtract: ["$latestYear", 1] },
              1
            ]
          },
          as: "year",
  ...
  {
    $sort: { "growthRate.rate": -1 }
  },
  { $limit: 5 },
  { $out: "EduCostStatQueryFour" }
])

This query should give you the top 5 states with the highest growth 
rate of overall expenses for Public In-State 4-year colleges. 
The output will be written to a new collection called "EduCostStatQueryFour".
